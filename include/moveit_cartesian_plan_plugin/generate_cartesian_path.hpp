...
public Q_SLOTS:
  ...
  //! Checks if the Way-Point is in the valid IK solution for the Robot.
	...
	void moveToJoint(std::vector<double> joints);					// TIEN VIET THEM
  //! Checks if the Way-Point is in the valid IK solution for the Robot.

	void getCurrentJoints();					// TIEN VIET THEM
	...
	void JointSpaceHandler(std::vector<double> joints);			// TIEN VIET THEM
	void JointSpaceUpdate();			// TIEN VIET THEM
...
Q_SIGNALS:
	...
	//void updateJoint(std::vector<double> current_joints);							// TIEN VIET THEM
	void updateJoint(double q1, double q2, double q3, double q4, double q5, double q6);
	void JointSpaceExecuteStarted();									// TIEN VIET THEM
	//! Let the RQT Widget know that Joint Space Execution has finished.

	void cartesianPathExecuteFinished();
	//! Send the percantage of successful completion of the Cartesian Path.

	void JointSpaceExecuteFinished();									// TIEN VIET THEM
	//! Send the percantage of successful completion of the Joint Space.
