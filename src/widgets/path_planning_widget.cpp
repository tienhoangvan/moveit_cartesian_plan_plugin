...
namespace moveit_cartesian_plan_plugin
{
	namespace widgets {
      ...
      */
      ...
      // Set ranges for spinbox and horizontalSlider            // TIEN VIET THEM
      ui_.joint_1->setRange(-360,360);    //ui_.horizontalSlider_1->setRange(-360,360);
      ui_.joint_2->setRange(-360,360);   // ui_.horizontalSlider_2->setRange(-360,360);
      ui_.joint_3->setRange(-360,360);    //ui_.horizontalSlider_3->setRange(-360,360);
      ui_.joint_4->setRange(-360,360);   // ui_.horizontalSlider_4->setRange(-360,360);
      ui_.joint_5->setRange(-360,360);    //ui_.horizontalSlider_5->setRange(-360,360);
      ui_.joint_6->setRange(-360,360);    //ui_.horizontalSlider_6->setRange(-360,360);
      ...
      connect(ui_.btnAddJoint_3,SIGNAL(clicked()),this,SLOT(getJoints()));             // TIEN VIET THEM

      connect(ui_.btnAddJoint_2,SIGNAL(clicked()),this,SLOT(jointHome()));                    // TIEN VIET THEM
      connect(ui_.btnAddJoint,SIGNAL(clicked()),this,SLOT(jointAddUI()));                    // TIEN VIET THEM
    ...
    void PathPlanningWidget::getJoints()                         // TIEN VIET THEM
    {
        ROS_INFO("Start getting current joint positions...");

        Q_EMIT getJoints_signal();

    }
    
    //void PathPlanningWidget::jointUpdate(std::vector<double> current_joints)                         // TIEN VIET THEM
    void PathPlanningWidget::jointUpdate(double q1, double q2, double q3, double q4, double q5, double q6)
    {
        /*! Function for updating joint states from Rviz to RQT Widget
        This function is connected to the Add Joints button 3 click() signal and sends the joint_goal(point_pos)
        */
        int q1d, q2d, q3d, q4d, q5d, q6d;
        /*
        // Joints States at Home
        q1d = current_joints[0];
        q2d = current_joints[1];
        q3d = current_joints[2];
        q4d = current_joints[3];
        q5d = current_joints[4];
        q6d = current_joints[5];
        */
        ...
        /*
        ROS_INFO("Joint %s: %f", "joint 1", q1d);
        ROS_INFO("Joint %s: %f", "joint 2", q2d);
        ROS_INFO("Joint %s: %f", "joint 3", q3d);
        ROS_INFO("Joint %s: %f", "joint 4", q4d);
        ROS_INFO("Joint %s: %f", "joint 5", q5d);
        ROS_INFO("Joint %s: %f", "joint 6", q6d);
        */
        // Add joint states to widget
        ui_.joint_1->setValue(q1d);  
        ui_.joint_2->setValue(q2d);
        ui_.joint_3->setValue(q3d);
        ui_.joint_4->setValue(q4d);  
        ui_.joint_5->setValue(q5d);
        ui_.joint_6->setValue(q6d);

        ROS_INFO("Updated Joints at Home...");

    }

    void PathPlanningWidget::jointHome()                         // TIEN VIET THEM
    {
        /*! Function for adding new joint states at Home Position from RQT Widget
        This function is connected to the Add Joints button 2 click() signal and sends the joint_goal(point_pos)
        */
        int q1d, q2d, q3d, q4d, q5d, q6d;
        // Joints States at Home
        q1d = 0;
        ...
        ROS_INFO("Adding Joints at Home...");

    }

    void PathPlanningWidget::jointAddUI()                         // TIEN VIET THEM
    {
        /*! Function for adding new joint state from RQT Widget
        This function is connected to the Add Joints button click() signal and sends the joint_goal(point_pos)
        */
        double q1, q2, q3, q4, q5, q6;
        
        q1 = DEG2RAD(ui_.joint_1->text().toDouble());
        q2 = DEG2RAD(ui_.joint_2->text().toDouble());
        q3 = DEG2RAD(ui_.joint_3->text().toDouble());
        q4 = DEG2RAD(ui_.joint_4->text().toDouble());
        q5 = DEG2RAD(ui_.joint_5->text().toDouble());
        q6 = DEG2RAD(ui_.joint_6->text().toDouble());

        std::vector<double> joints{q1, q2, q3, q4, q5, q6}; 

         ROS_INFO("Added Joints...");
        // // create joint goal
        Q_EMIT addJoint(joints);
...
void PathPlanningWidget::savePointsToFile()
{
    ...
    void PathPlanningWidget::JointSpaceStartedHandler()                 // TIEN VIET THEM
    {
      /*! Disable the RQT Widget when the Joint Space is executing.
      */
      ui_.tabWidget->setEnabled(false);
      ui_.targetPoint->setEnabled(false);
      ui_.btnAddJoint->setEnabled(false);
    ...
    void PathPlanningWidget::JointSpaceFinishedHandler()                // TIEN VIET THEM
    {
      /*! Enable the RQT Widget when the Joint Space execution is completed.
      */
      ui_.tabWidget->setEnabled(true);
      ui_.targetPoint->setEnabled(true);
      ui_.btnAddJoint->setEnabled(true);
