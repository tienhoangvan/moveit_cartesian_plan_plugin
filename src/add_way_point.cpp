...
void AddWayPoint::onInitialize()
{
    ...
    connect(widget_,SIGNAL(addJoint(std::vector<double>)),this,SLOT(addJointFromUI(std::vector<double>)));        // TIEN VIET THEM

    connect(path_generate,SIGNAL(updateJoint(std::vector<double>)),widget_,SLOT(jointUpdate(std::vector<double>)));   // TIEN VIET THEM
    ...
    connect(this,SIGNAL(Joint_signal(std::vector<double>)),path_generate,SLOT(JointSpaceHandler(std::vector<double>)));         // TIEN VIET THEM
    connect(widget_,SIGNAL(getJoints_signal()),path_generate,SLOT(JointSpaceUpdate()));        // TIEN VIET THEM
    ...
    connect(path_generate,SIGNAL(JointSpaceExecuteStarted()),widget_,SLOT(JointSpaceStartedHandler()));   // TIEN VIET THEM

    connect(path_generate,SIGNAL(cartesianPathExecuteFinished()),widget_,SLOT(cartesianPathFinishedHandler()));

    connect(path_generate,SIGNAL(updateJoint(double,double,double,double,double,double)),widget_,SLOT(jointUpdate(double,double,double,double,double,double)));   // TIEN VIET THEM

    connect(path_generate,SIGNAL(JointSpaceExecuteFinished()),widget_,SLOT(JointSpaceFinishedHandler()));   // TIEN VIET THEM
...
void AddWayPoint::addJointFromUI( std::vector<double> joints)
{
  /*! Function for handling the signal of the RQT to add a new Way-Point in the RViz enviroment.
  */
  ROS_INFO("Joints Added...");

  Q_EMIT Joint_signal(joints);
